...
%         \hline
%         (2) Neighborhood & \(\mathcal{O}(\bar{N}_i K)\) \\ 
%         \hline
%     \end{tabular}
...
\begin{itemize}
    ...
    % \item[(2)] The node \(i\) broadcasts \(\|\bar{\h}_i\|\), i.e., it transmits to all other nodes \(\{j \in \Mset \setminus i\}\) without direct links.
...
% For (1), the cost per node increases linearly with network size \(M\), whereas for communication scheme (2), the cost is linearly dependent on neighborhood size \(N_i\) and iteration count \(K\) only and therefore independent of network size.
...
% For (2), local complexity is \(\mathcal{O}(N_i^2 L^2)\), where \(N_i\) is the neighborhood size.
\vspace*{-0.6em}
\renewcommand{\arraystretch}{1.2}
\begin{table}[h]
    \centering
...
%     \item[(2)] The nodes only communicate with neighboring nodes, \(\Nset_i \subset \Mset\).
% \end{itemize}
% For (1) and (2), the cost per node increases linearly with network size \(M\): \(\mathcal{O}(M)\).
% Using communication scheme (2), as in \cite{yuDistributedBlindSystem2014,liuDistributedBlindIdentification2016} and proposed as an extension to \cite{blochbergerDBSI} in this paper, cost is linearly dependent on neighborhood size \(N_i\) and iteration count \(K\) only: \(\mathcal{O}(\bar{N}_i K)\).
% \begin{todo}
