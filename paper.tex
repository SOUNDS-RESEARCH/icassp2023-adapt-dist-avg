...
% For \(\gamma_i = 0\), only instantaneous neighborhood averages are used, which effectively leads to suboptimal computation, as \(\hat{\phi}_1^{n} = \hat{\phi}_2^{n} = \ldots = \hat{\phi}_M^{n}\) for large enough \(m\) will never be reached.
